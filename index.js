...
async function fetchStockData() {
    ...
    try {
        const stockData = await Promise.all(tickersArr.map(async (ticker) => {
            const url = `https://polygon-api-worker.nguyensongthienphuc.workers.dev/?ticker=${ticker}&startDate=${dates.startDate}&endDate=${dates.endDate}`
            ...
            if (status === 200) {
                ...
                throw new Error(`Polygon API Worker Error: ` + data)
            }
        }))
...
async function fetchReport(data) {
    ...
    try {
        const url = "https://openai-api-worker.nguyensongthienphuc.workers.dev"
